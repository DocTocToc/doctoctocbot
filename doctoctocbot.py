...
for tweet in timelineIterator:
    ...
    print("User in white list:", (useridstring in userIdWhiteList))
    if hasattr(tweet, 'retweeted_status'):
        isRetweet = True
        print("Is retweet:", isRetweet)
    print("Content:", tweet.text.encode('utf-8'))
    if ((useridstring in userIdWhiteList) and (not isRetweet)):
        oklist.append(tweet)
        print("Will be retweeted.")
    else:
        print("Will not be retweeted.")
...
print("Finished. {0} Tweets retweeted, {1} errors occured.".format(
