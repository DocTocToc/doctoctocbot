function encrypt() {
	return {
		then : function(callback) {
			...
			if (message_body_input.indexOf("-----BEGIN PGP MESSAGE-----") !== -1
					&& message_body_input.indexOf("-----END PGP MESSAGE-----") !== -1) {
				callback(true);
			} else {
				// encrypt message
				var keystatus = {
				...
				var readArmored = openpgp.key.readArmored(pubkey).keys;
				//var verifyPrimaryKey = readArmored.verifyPrimaryKey();
				...
				var options = {
					...
					publicKeys : readArmored
				};
				openpgp.encrypt(options).then(
						function(ciphertext) {
							var result = openpgp.message
									.readArmored(ciphertext.data);
							var validateMessage = JSON.stringify(result)
									.replace(/,/g, '\n');
							$('#check-message').text(validateMessage);
							message.value = ciphertext.data;
							var message_body = document
									.getElementById("message_body");
...
$(function() {
	...
	// init openpgp worker
	//var workerPath = "{% static "/gpgcontact/script/openpgp.worker.min.js" %}";
	...
	// show fingerprint
	//var fp = fingerprint(openpgp.key.readArmored($('#pubkey').text()).keys);
	//$("#fingerprint").text(fp);
